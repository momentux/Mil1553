...
void Process_6_1(const std::string dataString) {
	ICD_6_1_data data;
	...
	{
		data.velocitytimetag = std::stod(cell);
	}

	if (std::getline(lineStream, cell, ','))
	{
		data.northvelocity = std::stod(cell);
	}

	if (std::getline(lineStream, cell, ','))
	{
		data.eastvelocity = std::stod(cell);
	}

	if (std::getline(lineStream, cell))
	{
		data.downvelocity = std::stod(cell);
	}

	if (std::getline(lineStream, cell))
	{
		data.platformazimuth = std::stod(cell);
	}

	if (std::getline(lineStream, cell))
	{
		data.roll = std::stod(cell);
	}

	if (std::getline(lineStream, cell))
	{
		data.pitch = std::stod(cell);
	}

	if (std::getline(lineStream, cell))
	{
		data.trueheading = std::stod(cell);
	}

	if (std::getline(lineStream, cell))
	{
		data.magnecticheading = std::stod(cell);
	}
